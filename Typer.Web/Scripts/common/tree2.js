...
function Tree(params) {
    ...
    //Properties.
    self.mode = params.mode ? params.mode : MODE.SINGLE;
    self.visible = (params.hidden === true ? false : true);
    self.settings = {        
        expandWhenAddingNewNode: params.expandWhenAddingNewNode || true,
        doubleClickDelay: params.doubleClickDelay || 500,
        buttons: params.buttons || (params.container ? false : true) || false
    };
...
function DropArea(tree) {
    ...
    function release() {
        if (me.drop && me.drag) {
            ...
            if (me.drag === me.drop) {
                mielk.notify.display(dict.NodeCannotBeMovedToItself.get(), false);
            } else if (me.drag.isDescendant(me.drop)) {
                mielk.notify.display(dict.NodeCannotBeMovedToDescendant.get(), false);
            } else {
